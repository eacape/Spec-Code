...
class NewSpecWorkflowDialog(
    ...
    override fun doValidate(): ValidationInfo? {
        ...
        localReadinessSnapshot?.let { readiness ->
            SpecWorkflowOnboardingCoordinator.blockedEntryValidationMessage(
                ...
                return ValidationInfo(message, templateCombo)
            }
        }
    ...
    private fun refreshLocalReadiness() {
        localReadinessSnapshot = project?.let { activeProject ->
            LocalEnvironmentReadiness.inspect(activeProject)
        }
    }

    private fun selectedPrimaryEntry(): SpecWorkflowPrimaryEntry {
        return SpecWorkflowEntryPaths.primaryEntryForTemplate(selectedTemplate())
    }
    ...
    private fun updateTemplatePresentation(template: WorkflowTemplate, verifyEnabled: Boolean?) {
        ...
        templateStagesArea.toolTipText = presentation.stageMeaningSummary
